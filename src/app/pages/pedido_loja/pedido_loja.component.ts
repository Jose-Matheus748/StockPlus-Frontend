...
interface PedidoView extends PedidoExterno {
  _processando?: boolean;
  _acao?: 'concluir' | 'cancelar';
}
...
@Component({
  selector: 'app-pedido-loja',
  standalone: true,
  imports: [CommonModule, RouterModule, LayoutComponent],
  templateUrl: './pedido_loja.component.html',
})
export class PedidoLojaComponent implements OnInit {
  ...
  concluirPedido(pedido: PedidoView): void {
    if (!pedido.id || pedido._processando) return;

    pedido._processando = true;
    pedido._acao = 'concluir';
    this.erro = '';

    this.pedidoService.concluir(pedido.id).subscribe({
      next: (atualizado) => {
        pedido.status = atualizado.status;
        pedido._processando = false;
        pedido._acao = undefined;
      },
      error: (err) => {
        console.error(err);
        pedido._processando = false;
        pedido._acao = undefined;
        this.erro = err?.error?.message ?? 'Erro ao concluir pedido.';
      },
    });
  }
